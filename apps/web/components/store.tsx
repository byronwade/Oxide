...
import { ChevronLeft, ChevronRight, Star, Heart, Share2, Download, Play, Gift, TrendingUp, Clock, Zap, Target, Gamepad2, Users, MessageCircle, ThumbsUp, Eye, Globe, Map, Twitch, Youtube, MoreHorizontal, Settings, ExternalLink, BookmarkPlus, Calendar, Filter, SortAsc } from "lucide-react";
...
export function Store({ searchQuery, onGameSelect }: StoreProps) {
	...
	const featuredGames = [
		{
			...
			screenshots: ["/placeholder.svg?key=rn650", "/placeholder.svg?key=vr3w9", "/dragon-encounter.png", "/magic-spell.png"],
		...
		},
		{
			...
			screenshots: ["/cyberpunk-street.png", "/placeholder.svg?height=300&width=500", "/placeholder.svg?height=300&width=500", "/placeholder.svg?height=300&width=500"],
	...
	const topSellers = {
		...
		games: [
			{ id: 1, title: "Baldur's Gate 3", image: "/placeholder.svg?height=300&width=400", price: 59.99 },
			{ id: 21, title: "Lethal Company", image: "/placeholder.svg?height=300&width=400", price: 9.99 },
			{ id: 2, title: "Cyberpunk 2077", image: "/placeholder.svg?height=300&width=400", price: 29.99 },
			{ id: 20, title: "Valheim", image: "/placeholder.svg?height=300&width=400", price: 19.99 },
			{ id: 10, title: "Hogwarts Legacy", image: "/placeholder.svg?height=300&width=400", price: 59.99 },
		],
	};
	const recentlyUpdated = {
		...
		games: [
			...
			{ id: 20, title: "Valheim", image: "/placeholder.svg?height=300&width=400", updateInfo: "Ashlands Update" },
			{ id: 8, title: "Stardew Valley", image: "/placeholder.svg?height=300&width=400", updateInfo: "Update 1.6" },
			{ id: 19, title: "Apex Legends", image: "/placeholder.svg?height=300&width=400", updateInfo: "Season 21" },
			{ id: 17, title: "Sea of Thieves", image: "/placeholder.svg?height=300&width=400", updateInfo: "Season 12" },
		],
	};
	const browseCategories = [
		{ name: "Action", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "RPG", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Strategy", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Indie", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Adventure", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Simulation", icon: Gamepad2, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Sports & Racing", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
		{ name: "Puzzle", icon: Gift, image: "/placeholder.svg?height=200&width=400" },
	];
	...
	// Reusable Components
	const GameCard = ({ game, size = "normal" }: { game: Game; size?: "normal" | "small" | "large" }) => {
		const cardClass = size === "large" ? "w-80 h-96" : size === "small" ? "w-48 h-64" : "w-64 h-80";
		return (
			<Card className={`${cardClass} bg-zinc-900/80 border-zinc-700/50 hover:border-zinc-600 transition-all group overflow-hidden cursor-pointer flex-shrink-0`} onClick={() => onGameSelect(game)}>
				<CardContent className="p-0 h-full">
					<div className="relative h-full">
						<img src={game.coverImage || game.media?.coverImage || "/placeholder.svg"} alt={game.title} className="w-full h-full object-cover" />
						...
						{(game as any).discount && (
							<div className="absolute top-3 left-3">
								<Badge className="bg-green-600 text-white font-bold px-2 py-1">-{(game as any).discount}%</Badge>
							</div>
						)}
						{game.price === 0 && (
							<div className="absolute top-3 left-3">
							...
							<div className="flex items-center justify-between">
								...
								<div className="text-right">
									{game.price === 0 ? (
										...
										<div className="flex items-center gap-1">
											{game.originalPrice && <span className="text-xs text-zinc-400 line-through">${game.originalPrice}</span>}
	...
	const GameCarousel = ({ title, subtitle, icon: Icon, color, games }: { title: string; subtitle: string; icon: any; color: string; games: Game[] }) => (
		<div className="mb-12">
		...
		<div className="h-full overflow-auto bg-gradient-to-b from-zinc-950 to-black">
			...
			<div className="px-12 pt-8">
				<div className="flex items-center gap-4 mb-6">
					{heroTabs.map((tab) => (
						<Button key={tab.id} variant="ghost" onClick={() => setActiveHeroTab(tab.id)} className={`text-lg font-semibold transition-all duration-200 pb-2 border-b-2 ${activeHeroTab === tab.id ? "text-white border-blue-500" : "text-zinc-400 border-transparent hover:text-white"}`}>
							{tab.label}
						</Button>
					))}
				</div>
			...
			<div className="relative h-[600px] overflow-hidden">
				<div className="absolute inset-0">
					<img src={featuredGames[featuredIndex]?.heroImage || "/placeholder.svg"} alt={featuredGames[featuredIndex]?.title} className="w-full h-full object-cover" />
					<div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent" />
					<div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
				</div>
				<div className="relative h-full flex items-center px-12">
					<div className="max-w-2xl">
						<img src={featuredGames[featuredIndex]?.logoImage || "/placeholder.svg"} alt={featuredGames[featuredIndex]?.title} className="h-32 mb-6 object-contain" />
						...
						<div className="flex items-center gap-6 mb-6">
							<div className="flex items-center gap-2">
								...
								<span className="text-zinc-300">({featuredGames[featuredIndex]?.reviewCount?.toLocaleString()} reviews)</span>
							</div>
							<div className="text-zinc-300">Released: {featuredGames[featuredIndex]?.releaseDate}</div>
						</div>
						<div className="flex items-center gap-4">
							<div className="flex items-center gap-3">
								{featuredGames[featuredIndex]?.discount && <Badge className="bg-green-600 text-white text-lg px-3 py-1">-{featuredGames[featuredIndex].discount}%</Badge>}
								<div className="text-3xl font-bold text-white">
									${featuredGames[featuredIndex]?.price}
									{featuredGames[featuredIndex]?.originalPrice && <span className="text-lg text-zinc-400 line-through ml-3">${featuredGames[featuredIndex].originalPrice}</span>}
								</div>
							</div>
					...
					<div className="absolute right-12 top-1/2 transform -translate-y-1/2">
						<div className="grid grid-cols-2 gap-3">
							{featuredGames[featuredIndex]?.screenshots.map((screenshot: string, index: number) => (
								<div key={index} className="w-32 h-20 bg-zinc-800 rounded-lg overflow-hidden border border-zinc-700 hover:border-zinc-600 transition-colors cursor-pointer">
				...
				<div className="absolute bottom-6 left-12 flex items-center gap-4">
					<Button variant="ghost" size="sm" className="w-10 h-10 p-0 bg-black/50 hover:bg-black/70 text-white" onClick={() => setFeaturedIndex((prev) => (prev > 0 ? prev - 1 : featuredGames.length - 1))}>
						<ChevronLeft className="w-5 h-5" />
					</Button>
					<div className="flex gap-2">
						{featuredGames.map((_, index) => (
							<button key={index} className={`w-3 h-3 rounded-full transition-colors ${index === featuredIndex ? "bg-white" : "bg-white/40 hover:bg-white/60"}`} onClick={() => setFeaturedIndex(index)} />
						))}
					</div>
					<Button variant="ghost" size="sm" className="w-10 h-10 p-0 bg-black/50 hover:bg-black/70 text-white" onClick={() => setFeaturedIndex((prev) => (prev < featuredGames.length - 1 ? prev + 1 : 0))}>
				...
				<div className="my-16">
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
						{browseCategories.map((category) => {
							...
							return (
								<Card key={category.name} className="relative h-32 bg-zinc-900 border-zinc-800 hover:border-zinc-700 transition-all group overflow-hidden cursor-pointer">
									<img src={category.image || "/placeholder.svg"} alt={category.name} className="w-full h-full object-cover opacity-20 group-hover:opacity-30 transition-opacity" />
					...
					<Card className="bg-zinc-900/30 border-zinc-800/50">
						<CardContent className="p-6 flex gap-6">
							...
							<div className="flex-1">
								...
								<div className="p-4 bg-zinc-800/50 rounded-lg italic text-zinc-300 border-l-4 border-blue-500">"{curatorRecommendation.quote}"</div>
			...
			<div className="px-12 py-12">
				<div className="relative h-80 rounded-2xl overflow-hidden bg-gradient-to-r from-green-500 via-cyan-500 to-blue-500 p-12 flex items-center">
					...
					<div className="relative z-10">
						...
						<p className="text-white/80 text-xl mb-6 max-w-2xl">Get unlimited access to a library of incredible games, including day one releases, for one low monthly price.</p>
						<div className="flex items-center gap-4">
			...
			<div className="px-12 py-12">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					{themedCollections.map((collection) => {
						...
						return (
							<Card key={collection.title} className="relative bg-zinc-900/50 border-zinc-800/50 hover:border-zinc-700 transition-all group overflow-hidden cursor-pointer h-48">
								<CardContent className="p-0 h-full">
									<img src={collection.image || "/placeholder.svg"} alt={collection.title} className="w-full h-full object-cover opacity-30 group-hover:opacity-40 transition-opacity" />
			...
			<div className="px-12 py-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
					{entertainmentApps.map((app) => {
						const Icon = app.icon;
						return (
							<Card key={app.name} className="bg-zinc-900/50 border-zinc-800/50 hover:border-zinc-700 transition-all group overflow-hidden cursor-pointer">
